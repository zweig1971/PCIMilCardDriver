...
EXPORT_TYPE void MILDRIVER_Version_0p109_130204(void)
{
}

EXPORT_TYPE void MILDRIVER_VersionNr(BYTE* version)
{
	string StringVersion = "Version 1.09";
...
EXPORT_TYPE void MILDRIVER_Version_0p04S_130204(void)
{
}

EXPORT_TYPE void MILDRIVER_VersionNr(BYTE* version)
{
	string StringVersion = "Version 0.04S";
					 ...
					 )
{
	...
	if (ul_reason_for_call == DLL_PROCESS_ATTACH)
	{
		...
		SetPriorityClass(Process_ID,HIGH_PRIORITY_CLASS);
	}
...
//
EXPORT_TYPE DWORD PCI_PCIcardCount(void)
{
	int findCards = 0;
	CountUSBDevices(findCards);
	return(findCards);
}
...
//
EXPORT_TYPE DWORD PCI_DriverOpen(BYTE CardNr)
{
	int status = StatusOK;
...
EXPORT_TYPE DWORD PCI_IfkOnline(BYTE CardNr, BYTE IFCNr, BYTE &ReturnIFKNR ,DWORD &ErrorStatus)
{
...
EXPORT_TYPE DWORD PCI_IfkRead(BYTE CardNr, BYTE IFKAdress ,BYTE IFKFunktionCode, WORD &data, DWORD &ErrorStatus)
{
	DWORD status    = StatusOK;
...
EXPORT_TYPE DWORD PCI_IfkWrite(BYTE CardNr, BYTE IFKAdress, BYTE IFKFunktionCode, WORD data, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
EXPORT_TYPE DWORD PCI_FctCodeSnd(BYTE CardNr, BYTE IFKAdress, BYTE FunktionCode, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
//
EXPORT_TYPE DWORD PCI_PCIcardReset(BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
	...
	MilDriverArray[CardNr]->ResetUSBDev(status);
...
// zurück
EXPORT_TYPE DWORD PCI_ErrorString (DWORD status, BYTE* error_string)
{
	string ErrorMessage;
...
//        
EXPORT_TYPE DWORD PCI_TimerWait (BYTE CardNr, WORD Time, BYTE TimeBase, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
EXPORT_TYPE DWORD PCI_TimerSet (BYTE CardNr, WORD Time, BYTE TimeBase, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
// zurück
EXPORT_TYPE DWORD PCI_StatusRegRead (BYTE CardNr, DWORD &StatusReg, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
//
EXPORT_TYPE DWORD PCI_MilBusWrite (BYTE CardNr, WORD data, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
//
EXPORT_TYPE DWORD PCI_MilBusCMDWrite (BYTE CardNr, WORD cmd, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
//
EXPORT_TYPE DWORD PCI_IntMaskRead (BYTE CardNr, DWORD &data, DWORD &ErrorStatus)
{
	ErrorStatus = (ErrorStatus | IntMaskError);
	return(IntMaskError);
}

// Schreiben der Interrupt Maske
//
EXPORT_TYPE DWORD PCI_IntMaskWrite(BYTE CardNr, DWORD data , DWORD &ErrorStatus)
{
	ErrorStatus = (ErrorStatus | IntMaskError);
	return(IntMaskError);
}
...
//
EXPORT_TYPE DWORD PCI_IntEnable(BYTE CardNr, DWORD funcIntHandler, PVOID pUserDat, WORD Priority, DWORD &ErrorStatus)
{
	ErrorStatus = (ErrorStatus | FailedEnableIntr);
	return(FailedEnableIntr);
}

// Interrupt Abschalten
//
EXPORT_TYPE DWORD PCI_IntDisable(BYTE CardNr, DWORD &ErrorStatus)
{
	ErrorStatus = (ErrorStatus | FailedEnableIntr);
	return(FailedEnableIntr);
}
...
EXPORT_TYPE DWORD PCI_IntWait(BYTE CardNr, DWORD &IntAktiv, DWORD &ErrorStatus)
{
	ErrorStatus = (ErrorStatus | FailedEnableIntr);
	return(FailedEnableIntr);
}
...
//
EXPORT_TYPE DWORD PCI_IntAktiv(BYTE CardNr, DWORD &IntAktiv, DWORD &ErrorStatus)
{
	ErrorStatus = (ErrorStatus | FailedEnableIntr);
	return(FailedEnableIntr);
}
...
EXPORT_TYPE DWORD PCI_IntWaitStop(BYTE CardNr, DWORD &ErrorStatus)
{
	ErrorStatus = (ErrorStatus | FailedEnableIntr);
	return(FailedEnableIntr);
}
...
//
EXPORT_TYPE DWORD PCI_CntrlRegSet(BYTE CardNr, BYTE SetValue, DWORD &ErrorStatus)
{
	DWORD status	  = StatusOK;
...
//
EXPORT_TYPE DWORD PCI_CntrlRegRead(BYTE CardNr, BYTE &CntrlRegValue, DWORD &ErrorStatus)
{
	DWORD status	 = StatusOK;
	WORD EventCodeNr = 1;
	BYTE EVValue	 = 0;
...
//
EXPORT_TYPE DWORD PCI_EvFilterSet(BYTE CardNr, WORD EventCodeNr, BYTE SetEV, DWORD &ErrorStatus) 
{
	DWORD status = StatusOK;
...
//
EXPORT_TYPE DWORD PCI_EvFiFoRead (BYTE CardNr, WORD &EventCodeNr, bool &EVFifoNotEmpty, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
// zurueck
EXPORT_TYPE DWORD PCI_EvFilterRead(BYTE CardNr, WORD EventCodeNr, BYTE &FilterValue, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
// true oder false mit
EXPORT_TYPE bool PCI_StatusTest(BYTE CardNr, DWORD StatusBit, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
	bool Result;
	...
	Result = MilDriverArray[CardNr]-> TestUSBDevStatus (StatusBit, status);
...
//
EXPORT_TYPE DWORD PCI_MilBusRead (BYTE CardNr, WORD &data, bool &Fifo_not_empty, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
	DWORD ErrorStat = StatusOK;
...
EXPORT_TYPE DWORD PCI_EvTimerRead(BYTE CardNr, DWORD &Time ,DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
EXPORT_TYPE DWORD PCI_PulsOutSet(BYTE CardNr, WORD PulsBuchse, WORD EventOn, WORD EventOff, WORD PulsRahmen, bool OnOff, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
//
EXPORT_TYPE DWORD PCI_EvFilterClear(BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
//
EXPORT_TYPE DWORD PCI_EvFiFioClear(BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
//
EXPORT_TYPE DWORD PCI_SoftPuls1Start(BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
// Generiert per Software einen Puls
//
EXPORT_TYPE DWORD PCI_SoftPuls1Stop (BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
// Generiert per Software einen Puls
//
EXPORT_TYPE DWORD PCI_SoftPuls2Start(BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
// Generiert per Software einen Puls
//
EXPORT_TYPE DWORD PCI_SoftPuls2Stop (BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
EXPORT_TYPE DWORD PCI_SoftPuls3Start(BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
EXPORT_TYPE DWORD PCI_SoftPuls3Stop (BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
EXPORT_TYPE DWORD PCI_SoftPuls4Start(BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
EXPORT_TYPE DWORD PCI_SoftPuls4Stop (BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
EXPORT_TYPE DWORD PCI_SoftPuls5Start(BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
EXPORT_TYPE DWORD PCI_SoftPuls5Stop (BYTE CardNr, DWORD &ErrorStatus)
{
	DWORD status = StatusOK;
...
//
EXPORT_TYPE DWORD PCI_IntNextEnable(BYTE CardNr, DWORD &ErrorStatus)
{
	ErrorStatus = (ErrorStatus | GeneralError);
	return(GeneralError);
}
...
//
EXPORT_TYPE DWORD PCI_DriverClose(BYTE CardNr)
{
	int index = 0;
	int emptyIndex = 0;
